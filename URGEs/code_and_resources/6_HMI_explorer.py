...
dist_matrix = pd.read_csv("dist_matrix_100_full.csv", index_col=0)
...
app.layout = html.Div([
    html.H1("Historical Mutual Information Explorer"),
    
    html.Div([
        html.Label("Enter list of genes to be highlighted: "),
        html.Br(),
        html.Br(),
        dcc.Textarea(
            id='gene-input',
...
def save_graph_positions_and_clusters(G, pos, partition, file_path="community_graph.pickle"):
...
def update_graph(n_clicks, gene_input, threshold_value, stored_graph_data):
    ...
    if regenerate_graph:  # Only regenerate graph and recolor if the threshold changes.
        ...
        save_graph_positions_and_clusters(G, pos, partition)
    ...
    for k in G.nodes():
        ...
        # If the current node is one of the highlighted genes, it is colored lime
        if k.lower() in highlighted_genes:
            ...
            node_colors_highlighted.append("lime")
        else:
